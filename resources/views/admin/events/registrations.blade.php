    ...
    <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900">{{ $stats['sms_sent'] }}</p>
            </div>
                ...
                @forelse($registrations as $registration)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <button onclick="showRegistrationDetails({{ $registration->id }})" class="text-blue-600 hover:text-blue-900 mr-3">View</button>
                            <form action="{{ route('admin.events.registrations.update', $registration) }}" method="POST" class="inline">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="admin_notes" value="{{ $registration->admin_notes }}">
                                <input type="hidden" name="status" value="{{ $registration->status }}">
                                <button type="button" onclick="editRegistration({{ $registration->id }})" class="text-green-600 hover:text-green-900">Edit</button>
                            </form>
                        </td>
                    </tr>
